...
<tr>
	...
	<td align="right">
		{if (($allowedit && $order[0].status != 4) || ($order[0].owner == $uid && $order[0].status == 1))}
			<a onclick="removeLicenceFromOrder({$orderitem.personID},{$orderitem.orderID})" {popup caption="löschen" bgcolor="#FF0000" text="Diese Lizenz entfernen"} href="#"><img src="{$templateDir}/images/icons/basket_remove.png"></a>
		{/if}	
	</td>
</tr>
{/foreach}
